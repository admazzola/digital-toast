...
export default class AnimatedTexture extends THREE.CanvasTexture {
	...
	static async loadGif( fileLoader, url  )
	{
	 ...
	 async assign ( container ) {
		...
		if ( this.auto && this.index.indexOf( this ) == -1 )
		this.index.push( this );
		...
		for ( let frame of container.frames ) {
			...
			if ( !frame.image ) {
				...
				frame.image = await AnimatedTexture.copyCanvas().dataToImage( frame.patch, frame.dims.width, frame.dims.height );
	...
	dispose  () {
		...
		if ( this.auto ) {
			...
			const i = this.index.indexOf( this );
